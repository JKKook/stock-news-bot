...
# ── 빅테크 실적 발표 속보 ────────────────────────────────────────────
#   빅테크(국내/해외)+관심종목의 '분기 실적 발표' 결과가 나오면 AI로 2~3문장 요약 + 원문 링크(한 줄).
#   탐지: 대상 종목명으로 구글뉴스를 '분기 발표 결과' 키워드로 검색(프리뷰·전망 제외). 회사별 중복 억제.
#   비용 절감: 매 실행 EARNINGS_SCAN_BATCH개씩 순번(cursor)으로 돌며 검색 — 실적 헤드라인은 몇 시간 지속돼 순환으로도 포착.
EARNINGS_ALERT_ENABLE = True
EARNINGS_SCAN_BATCH = 8            # 한 실행에서 검색할 대상 수(순번 순환) — RSS 호출량 상한
EARNINGS_MAX_PER_RUN = 3          # 한 실행에서 보낼 최대 실적 속보 수
EARNINGS_FRESH_HOURS = 12         # 이 시간 내 '발표 결과' 기사만(오늘 발표 포착)
EARNINGS_DUP_DAYS = 3             # 같은 회사 실적은 N일 내 1회만(같은 분기 재알림 억제)
# 빅테크 큐레이션 — (표시명, 야후심볼, 검색베이스, 언어, 지역). 관심종목(TICKERS)과 합쳐 대상 구성(중복 제거).
#   심볼은 실적 발표 기업의 '자기 주가 등락'을 속보에 붙이기 위함(지수 반응 대신).
EARNINGS_BIGTECH = [
    ("애플",         "AAPL",       "Apple",      "en", "해외"),
    ("엔비디아",      "NVDA",       "Nvidia",     "en", "해외"),
    ("마이크로소프트", "MSFT",       "Microsoft",  "en", "해외"),
    ("알파벳",       "GOOGL",      "Alphabet Google", "en", "해외"),
    ("아마존",       "AMZN",       "Amazon",     "en", "해외"),
    ("메타",         "META",       "Meta Platforms",  "en", "해외"),
    ("테슬라",       "TSLA",       "Tesla",      "en", "해외"),
    ("브로드컴",      "AVGO",       "Broadcom",   "en", "해외"),
    ("TSMC",        "TSM",        "TSMC",       "en", "해외"),
    ("삼성전자",      "005930.KS",  "삼성전자",     "ko", "국내"),
    ("SK하이닉스",    "000660.KS",  "SK하이닉스",   "ko", "국내"),
]
EARNINGS_RESULT_KW = [            # '결과' 신호(프리뷰 아님) — 하나 이상 있어야 실적 발표로 인정
